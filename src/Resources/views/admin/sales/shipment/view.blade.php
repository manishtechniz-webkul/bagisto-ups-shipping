<x-admin::layouts>
    ...
    @php
        ...
        $ups = app(\Webkul\UpsShipping\Carriers\Ups::class);
    @endphp
    ...
    @pushOnce('scripts')
        <script type="text/x-template" id="v-payment-shipment-template">
            <!-- Component 3 -->
            <x-admin::accordion>
                <x-slot:header>
                    <p class="text-gray-600 dark:text-gray-300 text-base  p-2.5 font-semibold">
                ...
                <x-slot:content>
                    ...
                    <div class="pt-4">
                        ...
                        @if (is_integer(stripos($order->shipping_method, $ups->getMethod())))
                            <p
                                class="pt-4 text-gray-800 dark:text-white font-semibold"
                                v-if="shipmentTrackingLoading"
                            >
                                @{{ shipmentTracking?.shipmentStatus }}
                            </p>

                            <template v-else>
                                <x-ups::admin.shimmer.shipment.status/>
                            </template>

                            <p class="text-gray-600 dark:text-gray-300">
                                @lang('ups::app.admin.sales.shipment.view.shipment-status')
                            </p>
                            ...
                            <p
                                ...
                                v-if="shipmentTrackingLoading"
                            >
                                @{{ shipmentTracking?.shipmentDescription }}
                            </p>

                            <template v-else>
                                <x-ups::admin.shimmer.shipment.description/>
                            </template>

                            <p class="text-gray-600 dark:text-gray-300">
                                @lang('ups::app.admin.sales.shipment.view.shipment-description')
                            </p>
                        @endif
                    </div>
                </x-slot>
        ...
        <script type="module">
            app.component('v-payment-shipment', {
                template: '#v-payment-shipment-template',
                ...
                methods: {
                    trackUpsShipment() {
                         ...
                         * Todo:: verify working 
                         */
                        this.$axios.get("{{ route('admin.ups_shipping.order.track', $shipment->track_number) }}")
                            .then((response) => {
                                this.shipmentTrackingLoading = true;

                                let trackResponse = response.data.result.trackResponse;

                                shipmentTracking['shipmentStatus'] = trackResponse.shipment[0]['currentStatus']['simplifiedTextDescription'];

                                shipmentTracking['shipmentDescription'] = trackResponse.shipment[0]['currentStatus']['description'];
                            })
                            .catch(error => {
                            });
                    },
